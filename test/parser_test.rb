...
class ParserTest < Test::Unit::TestCase
  ...
  def test_parse_identifier
    assert_equal({:conditions => 'name = "kuli"'}, @parser.parse_arql('name = kuli').find_options(User))
  end
  ...
  def test_parse_identifier_quated_by_double_quate_including_escaped_double_quate
    assert_equal({:conditions => "name = \"ku\\\"li\""}, @parser.parse_arql('name = "ku\"li"').find_options(User))
  end
